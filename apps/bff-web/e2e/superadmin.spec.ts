...
test("/superadmin muestra la visión global de plataforma", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Superadministración" })).toBeVisible({
    timeout: 15000,
  });
  ...
  await expect(
    page
      .getByText(/empresas|usuarios|sanciones|flota|importe total|visión global|totales/i)
      .first(),
