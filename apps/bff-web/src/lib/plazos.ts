...
export function calcularPlazos(entrada: EntradaPlazos): PlazoCalculado[] {
  ...
  for (const regla of reglasAplicables(entrada)) {
    ...
    if (baseFecha) {
      ...
      else if (regla.tipoDia === "hábiles")
        calculada = sumarDiasHabiles(baseFecha, regla.dias ?? 0);
    ...
    if (doc && calculada) {
      ...
      if (!regla.fiable)
        notes = "Cálculo orientativo: debe contrastarse con el plazo indicado en la notificación.";
    } else {
