...
const PISTAS_TIPO: Array<{ tipo: string; palabras: RegExp }> = [
  ...
  {
    tipo: "Manipulación de tacógrafo",
    palabras: /manipulaci[oó]n|im[aá]n|tac[oó]grafo\s+(manipul|alterad)/i,
  },
  {
    tipo: "Exceso de tiempo de conducción",
    palabras: /tiempo(s)? de conducci[oó]n|conducci[oó]n (ininterrumpida|diaria|continuada)/i,
  },
  ...
  {
    tipo: "Acceso a zona restringida",
    palabras: /zona de bajas emisiones|zbe|zona restringida|acceso restringido|[aá]rea central/i,
  },
  {
    tipo: "Identificación del conductor",
    palabras: /identificaci[oó]n del conductor|no identificar/i,
  },
  { tipo: "ITV", palabras: /\bitv\b|inspecci[oó]n t[eé]cnica/i },
  { tipo: "Seguro", palabras: /seguro obligatorio|p[oó]liza|sin seguro/i },
  {
    tipo: "Falta de documentación",
    palabras:
      /documentaci[oó]n|autorizaci[oó]n de transporte|tarjeta de transporte|carta de porte/i,
  },
...
export function validarAntesDeCrear(
  ...
  if (esTipoGenerico(comoTexto(campos["tipo_infraccion"]?.valor))) {
    incidencias.push({
      ...
      mensaje:
        "El tipo de infracción es demasiado genérico. Concreta el tipo antes de crear el expediente.",
      nivel: "aviso",
    });
  }
  ...
  if (comoTexto(campos["importe_original"]?.valor) && (!Number.isFinite(importe) || importe <= 0)) {
    incidencias.push({
      ...
      mensaje: "El importe no es un valor válido.",
      nivel: "aviso",
    });
  }
  ...
  if (comparacion.nombreComparable && !comparacion.nombreCoincide) {
    incidencias.push({
      clave: "razon_social",
      mensaje:
        "La razón social del documento difiere de la de la empresa activa (comprobación orientativa).",
      nivel: "aviso",
    });
  }
