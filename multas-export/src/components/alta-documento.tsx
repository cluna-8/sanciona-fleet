...
const PASOS = ["Documento recibido", "Procesando documento", "Revisar información", "Expediente creado"];
...
function normalizarCampos(entrada: Campos): Campos {
  ...
  if (bruto === true || String(bruto).trim().toLowerCase() === "true" || String(bruto).trim().toLowerCase() === "sí")
    identificacion = "Sí";
  else if (
    bruto === false ||
  ...
  const tipoActual = campos["tipo_infraccion"]?.valor == null ? "" : String(campos["tipo_infraccion"].valor);
  if (esTipoGenerico(tipoActual)) {
    ...
    if (deducido) {
      campos["tipo_infraccion"] = { valor: deducido, confianza: "Medio", fuente: "Deducido de los hechos" };
    }
  }

  const tipoFinal = campos["tipo_infraccion"]?.valor == null ? "" : String(campos["tipo_infraccion"].valor);
  const categoriaActual = campos["categoria"]?.valor == null ? "" : String(campos["categoria"].valor);
...
export function AltaDesdeDocumento({
  ...
  const comparacion: ComparacionEmpresa = compararEmpresa(empresa, {
    razonSocial: campos["razon_social"]?.valor == null ? null : String(campos["razon_social"].valor),
  ...
  function actualizar(clave: string, valor: string) {
    setCampos((prev) => ({
      ...prev,
      [clave]: { valor, confianza: prev[clave]?.confianza ?? "Alto", fuente: prev[clave]?.fuente ?? null },
    }));
  }
    ...
    <div className="card-surface p-5">
      ...
      <ol className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-xs">
        {PASOS.map((p, i) => (
          ...
          >
            {i < paso ? <CheckCircle2 className="h-3.5 w-3.5" /> : <span className="text-[10px]">{i + 1}.</span>}
            {p}
          </li>
        ))}
      </ol>
      ...
      {estado && paso < 2 && (
        <p className="mt-3 text-xs text-muted-foreground">Estado: {estado}</p>
      )}
        ...
        <div className="mt-5 space-y-4">
            ...
            <div className="border border-destructive/40 bg-destructive/5 p-4">
              ...
              <dl className="mt-2 grid gap-1 text-xs sm:grid-cols-2">
                ...
                <div>
                  ...
                  <dd className="font-medium">{comparacion.cifDocumento || "Pendiente de confirmar"}</dd>
                </div>
              </dl>
              ...
              {confirmado && (
                <p className="mt-2 text-xs text-muted-foreground">
                  Has confirmado continuar. La discrepancia quedará registrada en el historial del expediente.
                </p>
              )}
            </div>
          )}

          <p ref={revisionRef} className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {clavesVisibles.map((clave) => {
              ...
              const valor = campo?.valor === null || campo?.valor === undefined ? "" : String(campo.valor);
              const opciones =
                clave === "requiere_identificacion_conductor"
                  ...
                  : clave === "tipo_infraccion"
                    ...
                    : clave === "categoria"
                      ...
                      : null;
              const listaOpciones =
                opciones && valor && !opciones.includes(valor as never) ? [valor, ...opciones] : opciones;
              return (
                <div key={clave} className="space-y-1">
                  ...
                  {listaOpciones ? (
                    <Select
                      value={valor || (clave === "requiere_identificacion_conductor" ? "Pendiente de confirmar" : "")}
          ...
          {incidencias.length > 0 && (
            <div className="border border-accent/40 bg-accent/10 p-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Puedes corregir los datos arriba antes de continuar. No se completa ningún dato automáticamente.
              </p>
            </div>
          )}
          ...
          {discrepanciaCif && !confirmado && (
            <p className="text-xs text-muted-foreground">
              Confirma la discrepancia de CIF con «Continuar de todos modos» para poder crear el expediente.
            </p>
          )}
        </div>
      )}
