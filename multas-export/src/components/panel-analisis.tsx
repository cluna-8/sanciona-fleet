...
export function PanelAnalisis({ sanctionId, puedeGestionar, esRevisor, userId }: Props) {
  ...
  const revisar = useMutation({
    mutationFn: async (cambios: { recommendation?: string; review_notes?: string; validar?: boolean }) => {
      ...
      const { error } = await supabase.from("sanction_analyses").update(payload as never).eq("id", analisis.id);
  ...
  return (
    <div className="card-surface p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        {puedeGestionar && (
          <Button size="sm" variant="outline" onClick={() => analizar.mutate()} disabled={analizar.isPending}>
            {analizar.isPending ? (
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Recomendación preliminar basada en los datos del expediente. Requiere validación profesional.
      </p>

      {/* Plazos */}
      <div className="mt-4">
        <div className="flex items-center justify-between">
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Plazos</p>
          {puedeGestionar && (
            <button
              type="button"
      ...
      {!isLoading && !analisis && (
        <p className="mt-4 rounded-lg border border-dashed border-border p-4 text-sm text-muted-foreground">
          Este expediente todavía no se ha analizado. Inicia el análisis para obtener la recomendación
          preliminar, la documentación pendiente y los factores detectados.
        </p>
      )}
        ...
        <div className="mt-5 space-y-4">
          <div className="rounded-lg border border-border p-4">
            <div className="flex flex-wrap items-center gap-2">
              <span className={`rounded-full border px-2 py-0.5 text-[11px] ${CLASES_SEMAFORO[semaforo]}`}>
          ...
          {puedeGestionar && (
            <div className="flex flex-wrap gap-2">
              <Button size="sm" onClick={() => crearBorrador.mutate("Alegaciones")} disabled={crearBorrador.isPending}>
...
function Bloque({ titulo, children }: { titulo: string; children: React.ReactNode }) {
  return (
    <div className="mt-4">
      <p className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{titulo}</p>
      {children}
    </div>
  );
}
