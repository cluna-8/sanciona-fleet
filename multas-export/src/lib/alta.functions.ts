...
export const crearAlta = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await supabaseAdmin
      .from("profiles")
      .upsert(
        { id: userId, email, full_name: data.contacto, username: usuario, phone: data.telefono || null },
        { onConflict: "id" },
      );
    ...
    if (!organizationId) {
      ...
      await supabaseAdmin.from("organizations").update({ plan: data.plan }).eq("id", organizationId);
    }
