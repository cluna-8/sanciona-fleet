...
function Informes() {
  ...
  const bloques = useMemo(() => {
    ...
    return [
      { titulo: "Por estado", nombre: "sanciones-por-estado", filas: agrupar(lista, (s) => s.status) },
      {
      ...
      },
      {
        ...
        filas: agrupar(lista, (s) => (s.notification_date ?? s.created_at).slice(0, 7)).sort((a, b) =>
  ...
  const ahorro = abiertos.reduce(
    (t, s) =>
      t +
      Math.max(0, Number(s.original_amount ?? 0) - Number(s.discounted_amount ?? s.original_amount ?? 0)),
    0,
  );
