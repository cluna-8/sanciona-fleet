...
import { useSanciones, useVehiculos, useConductores, type Sancion } from "@/hooks/use-datos";
...
function ListadoSanciones() {
  ...
  const organismos = useMemo(
    () => [...new Set((sanciones ?? []).map((s) => s.sanctioning_authority).filter(Boolean))] as string[],
    [sanciones],
  );
  ...
  }, [sanciones, estado, vehiculo, conductor, organismo, categoria, prioridad, desde, hasta, busqueda]);
    ...
    >
      <div className="card-surface p-5">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <FiltroSelect label="Estado" valor={estado} onChange={setEstado} opciones={ESTADOS_SANCION.map((e) => ({ value: e, label: e }))} />
          <FiltroSelect
            label="Matrícula"
            valor={vehiculo}
              ...
              <tbody>
                {filtradas.map((s) => (
                  <tr key={s.id} className="border-t border-border hover:bg-secondary/40">
                    ...
                    <td className="px-5 py-3 font-medium">{s.vehicles?.registration_number ?? "—"}</td>
